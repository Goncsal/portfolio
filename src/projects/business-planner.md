...
Project Summary: A full-stack web app for building Portuguese IAPMEI business plans — a report editor plus a financial engine that reproduces the official IAPMEI Excel template cell-for-cell, with real-time co-editing and PDF/Word export.

## What it is

Business Planner came out of a real annoyance: in Portugal, IAPMEI business plans get built in one enormous, fragile Excel template that economists and consultants pass around and break constantly. I wanted to replace that with a web app where the writing and the numbers live in the same place — and where the numbers actually match the official template.
...
This was the hard part, and the reason the project exists. The official IAPMEI template is around 2,800 formulas spread across 15 sheets. I extracted the whole thing to JSON, translated every Excel cell reference into a semantic id like `pressupostos.total_de_vendas_e_servicos_prestados[2025]`, and rebuilt it server-side.
